...
class FC_DGCNN(nn.Module):
    def __init__(self, xdim, num_out, nclass=3):
        super(FC_DGCNN, self).__init__()
        self.BN1 = nn.BatchNorm1d(xdim[2])  # 对第二维（第一维为batch_size)进行标准化
        self.fc1 = Linear(xdim[1] * num_out, 32)
    ...
    def forward(self, x, DGCNN_result):
        # print(x.shape)
        # print(DGCNN_result.shape)
        result = DGCNN_result.reshape(x.shape[0], -1)
        result = F.relu(self.fc1(result))
