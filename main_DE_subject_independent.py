...
def train(train_iter, test_iter, model, criterion, optimizer, num_epochs, sub_name):
    ...
    for ep in range(num_epochs):
        ...
        print('>>> best test.py Accuracy: {}'.format(acc_test_best))
...
def main_LOCV():
    dir = 'E:/SynologyDrive/PhD0/SEED/DE/session1/'  # 04-0.9916， 0.86
    ...
    for sub_i in range(3, sub_num):
        ...
        torch.save(model.state_dict(), test_list)

    print('save...')
    scio.savemat('./result/acc_all/acc_de_SEED_LOCV.mat',
                 {'acc_all': acc_all, 'sub_list': np.array(file_list, dtype=np.object)})

    print('>>> LOSV test.py acc: ', acc_all)
    print('>>> LOSV test.py mean acc: ', acc_mean)
    print('>>> LOSV test.py std acc: ', np.std(np.array(acc_all)))
